...
public class DownloadTask extends Handler {
    ...
    private volatile boolean isSuportMultithread = false;
    ...
    private static final int MSG_SINGLE =5;
    private int pro;
    private static final int MSG_SINGLE_FINISH=6;
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_SINGLE_FINISH:
                mListner.onFinished();
                break;
            case MSG_SINGLE:
                mListner.onProgress(pro);
                break;
            case MSG_PROGRESS://进度
                ...
                mListner.onProgress((int)((progress * 1.0f / mFileLength)*100));
                break;
            case MSG_PAUSE://暂停
    ...
    public synchronized void start() {
        final long range=0;// 预留断点续传的
        try {
            Log.e(TAG, "start: " + isDownloading + "\t" + mPoint.getUrl());
            if (isDownloading) {return;}
            isDownloading = true;
            //String url ="http://gykj123.cn:9025/downLoadApp/协保通协保员/release";
            String url ="https://ram-public.oss-cn-zhangjiakou.aliyuncs.com/appdownload/xbtxby/1.0.5/%E5%8D%8F%E4%BF%9D%E9%80%9A%28xby%29_beta_105_v_1.0.5_20190911112104.apk";
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    //单线程下载
                        ...
                        try {
                            ...
                            String filePath = mPoint.getFilePath() ;
                            File file = new File(filePath, mPoint.getFileName());
                            ...
                            while ((len = inputStream.read(buf)) != -1) {
                                ...
                                total += len;
                                lastProgress = pro;
                                pro = (int) (total * 100 / randomAccessFile.length());
                                if (pro > 0 && pro != lastProgress) {
                                    sendEmptyMessage(MSG_SINGLE);
                                }
                            }
                            sendEmptyMessage(MSG_SINGLE_FINISH);
                        } catch (Exception e) {
                            isDownloading = false;
                            e.printStackTrace();
                        } finally {
    ...
    public void setSuportMultithread(boolean isSuportMultithread){
        this. isSuportMultithread = isSuportMultithread;
    }
}
