...
public class MainAdapter extends RecyclerView.Adapter<MainAdapter.MainVH> {
    private ArrayList<GrandPhotoBean> list;
    private LayoutInflater mInflater;
    private RequestManager mGlide;

    MainAdapter(Context cxt, ArrayList<GrandPhotoBean> list) {
        this.list = list;
    ...
    public MainVH onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MainVH(mInflater.inflate(R.layout.main_home_adapter_item, parent, false));
    }

    @Override
    public void onBindViewHolder(MainVH holder, int position) {
        GrandPhotoBean photo = list.get(position);
