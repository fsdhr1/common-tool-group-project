...
public class Camera1Control implements ICameraControl {
    ...
    private boolean mIsFindOptSize;//是否已经查找过合适的尺寸 查找过程耗时优化
    private boolean mIsFindAdjustSize;//是否已经找到合适的尺寸
    ...
    public void takePicture(final OnTakePictureCallback onTakePictureCallback) {
        ...
        try {
           // Camera.Size picSize = getOptimalSize(camera.getParameters().getSupportedPictureSizes());
            //Camera.Size picSize = camera.new Size(1920,1080);
    ...
    private void opPreviewSize(int width, @SuppressWarnings("unused") int height) {
        ...
        if (parameters != null && camera != null && width > 0) {
            if(!mIsFindOptSize){
                Set<Camera.Size> supportedPictureSizes = new HashSet<>(camera.getParameters().getSupportedPictureSizes());
                Set<Camera.Size> supportedPreviewSizes = new HashSet<>(camera.getParameters().getSupportedPreviewSizes());
                supportedPreviewSizes.retainAll(supportedPictureSizes);
                if(supportedPreviewSizes.size() > 0){
                    optSize = getOptimalSize(new ArrayList<>(supportedPreviewSizes));
                    mIsFindAdjustSize = true;
                }
                mIsFindOptSize = true;
            }
            
            if(optSize == null){//未找到合适的尺寸
                optSize = getOptimalSize(camera.getParameters().getSupportedPreviewSizes());
            }
            
            if(mIsFindAdjustSize) {
                parameters.setPictureSize(optSize.width,optSize.height);
            }else {
                Camera.Size optimalSize = getOptimalSize(camera.getParameters().getSupportedPictureSizes());
            ...
            parameters.setRotation(getSurfaceOrientation());
            stopPreview();
            try {
                camera.setParameters(parameters);
            } catch (RuntimeException e) {
