...
import com.gykj.base.event.ItemClickListener;
import com.gykj.utils.ObjectUtil;
...
public abstract class BaseRecycleAdapter<M, H extends BaseViewHolder<M>> extends RecyclerView.Adapter<H> {
     ...
     * @param vars
     */
    public void removeItemByVar(M model, String... vars) {
        if (vars == null) return;
        Map<String, String> varLinkMap = new HashMap<>();
        for (String var : vars) {
            varLinkMap.put(var, var);
        }
        int index = -1;
        for (M m : dataList) {
            index++;
            if (ObjectUtil.objFieldsIsEqual(m, model, varLinkMap)) {
                break;
            }
        }
        removeItem(index);
    ...
    public abstract void isPosItemVisible(int firstItemPosition, int lastItemPosition);

    public abstract void scrollState(RecyclerView recyclerView, int newState);
